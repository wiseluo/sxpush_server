...
use oppo_push\Http\Http;
use oppo_push\Http\Request;
use oppo_push\Http\Response;
...
class Push
{
    ...
    private $unicast_url = 'https://api.push.oppomobile.com/server/v1/message/notification/unicast';
    ...
    public function getAuthToken()
    {
        ...
        if(!$auth_token){
            throw  new \Exception("获取 auth_token 失败:". $this->_authTokenInfo['message']);
            return null;
        }
        return $auth_token;
    }
    ...
    private function check()
    {
        ...
        if(!empty($registration_id) && is_array($registration_id)){
            ...
            $registration_id = array_filter($registration_id);
        }
    ...
    public function send()
    {
        ...
        $this->registration_id = array_filter($this->registration_id);
        if (count($this->registration_id) >= 2 && count($this->registration_id) <= 1000) {
            return $this->broadcast();
        } else if (count($this->registration_id) == 1) {
            $this->registration_id = array_pop($this->registration_id);
            return $this->unicast();
        } else {
            throw new \Exception("oppo超过推送通道单次最大设备数:1000");
        }
    }

    private function unicast()
    {
        $data = [
            'message' => json_encode([
                'target_type' => 2,
                'target_value' => $this->registration_id,
                'notification' => $this->build(),
            ]),
            'auth_token' => $this->auth_token,
        ];
        
        $response = $this->_http->post($this->unicast_url, array(
            'data' => $data
        ));
    ...
    private function broadcast()
    {
        ...
        $data = array(
            ...
            'target_type' => 2,
            'target_value' => implode(';', $this->registration_id),
        );
