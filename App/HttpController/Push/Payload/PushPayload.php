...
class PushPayload {
    ...
    public function setPlatform($platform) {
        ...
        if (is_string($platform)) {
            ...
            if ('all' === $ptf) {
                $this->platform = self::$EFFECTIVE_DEVICE_TYPES;
    ...
    public function build() {
        ...
        if (in_array('ios', $payload["platform"]) && !array_key_exists('ios', $payload['notification'])) {
            throw new InvalidArgumentException('ios notification can not be null');
        }
        if (in_array('android', $payload["platform"]) && !array_key_exists('android', $payload['notification'])) {
            throw new InvalidArgumentException('android notification can not be null');
        }
    ...
    public function androidNotification($alert = '', array $notification = array()) {
        ...
        $android['title'] = isset($notification['title']) ? $notification['title'] : '消息通知';
        if (!empty($notification)) {
